...
uint64_t myxs64_splitmix64(uint64_t& x) {
...
__device__ __forceinline__
void myxs64_init_rng(MyXS64State& st, uint64_t seed, int tid) {
...
__device__ __forceinline__
float2 myxs64_normal2(MyXS64State& st) {
...
__device__ __forceinline__
float4 myxs64_normal4(MyXS64State& st) {
    float2 pair1 = myxs64_normal2(st);
    float2 pair2 = myxs64_normal2(st);
