...
uint64_t mysm64_step(uint64_t& x) {
...
__device__ __forceinline__
void mysm64_init_rng(MySM64State& st, uint64_t seed, int tid) {
...
__device__ __forceinline__
float2 mysm64_normal2(MySM64State& st) {
...
__device__ __forceinline__
float4 mysm64_normal4(MySM64State& st) {
