...
__device__ uint64_t mypcg32_splitmix64(uint64_t& x) {
...
__device__ void mypcg32_init_rng(MyPCG32State& st, uint64_t seed, int tid) {
    uint64_t x = seed + (uint64_t)tid; 
    st.state = mypcg32_splitmix64(x);

    //Init inc
    // Usiamo una costante di mixing per l'incremento
    uint64_t h = (uint64_t)tid + 0x853221AF52AD5327ULL;
    h ^= h >> 33;
    h *= 0xff51afd7ed558ccdULL;
    st.inc = h | 1; // Fondamentale: deve essere dispari
    
    // Passo di avanzamento
    st.state = st.state * 6364136223846793005ULL + st.inc;
}
