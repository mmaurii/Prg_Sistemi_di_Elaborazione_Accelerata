...
__device__ void pcg32_seed(MyPCG32* rng, uint64_t initstate, uint64_t initseq) {
    ...
    for (int i = 0; i < 10; i++) pcg32_random(rng);
    rng->state += initstate;
    // decorrelazione
    pcg32_random(rng);
}
...
__global__ void generate(uint64_t* out, int n, uint64_t seed) {
    ...
    uint64_t s = splitmix64(seed ^ tid);
    uint64_t seq = splitmix64(tid);

    pcg32_seed(&rng, s, seq);
...
int main(void) {
    ...
    // Scrittura binaria
    std::ofstream file("pcg32_s64.bin", std::ios::binary);
    ...
    std::cout << "File generato: pcg32_s64.bin (" << N << " uint64)\n";
}